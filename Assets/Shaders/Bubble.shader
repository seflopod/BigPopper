...
#pragma exclude_renderers gles
		...
		void vert (inout appdata_full v, out Input o)
		{
			...
			float4 waveVec = WaveDirX * v.texcoord.x + WaveDirY * v.texcoord.y;
			waveVec += _Time.y * WaveSpeed + WaveOffset;
			waveVec = frac(waveVec);
			
			waveVec -= 0.5;
			waveVec *= 2*3.14159;
			
			float3 positionDeformation = v.normal*dot(sin(waveVec), WaveHeight);
			float3 positionDeformedObject = v.vertex+positionDeformation;
			...
			float3 warping = binormal * binormalWarping + v.tangent * tangentWarping;
			float3 normalDeformed = v.normal + warping;
			float3 normalDeformedWorld = normalize(mul(normalDeformed, (float3x3)_Object2World));
			float3 view = normalize(_WorldSpaceCameraPos - mul(float4(positionDeformedObject, 1), (float4x3)_Object2World));
			float oDot = dot(normalDeformedWorld, view);
			
			o.screenPos = mul(float4(positionDeformedObject, 1), UNITY_MATRIX_VP);
			o.viewDir = view;
			o.uv_Texture = v.texcoord;
			o.worldRefl = 2 * oDot * normalDeformedWorld - view;
